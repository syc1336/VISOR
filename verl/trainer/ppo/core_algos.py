...
def compute_policy_loss(old_log_prob, log_prob, advantages, eos_mask, cliprange):
    ...
    """
    # Sanitize log ratio to avoid NaN / inf in ratio computation:
    #   -inf - (-inf) = NaN  →  nan_to_num → 0  →  ratio = 1  (no update, correct)
    #   finite - (-inf) = +inf  →  clamp → +10  →  ratio ≈ 22026  (large but finite)
    #   -inf - finite = -inf  →  clamp → -10  →  ratio ≈ 0  (correct)
...
def kl_penalty(logprob: torch.FloatTensor, ref_logprob: torch.FloatTensor, kl_penalty) -> torch.FloatTensor:
    ...
    if kl_penalty == 'low_var_kl':
        ...
        kl = torch.nan_to_num(kl, nan=0.0, posinf=10.0, neginf=-10.0)
        ratio = torch.exp(kl)
